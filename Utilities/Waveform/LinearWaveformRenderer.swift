...
public struct LinearWaveformRenderer: WaveformRenderer {
    ...
    public func render(samples: [Float], on context: CGContext, with configuration: Waveform.Configuration, lastOffset: Int, duration: Float) {
        ...
        if case .striped = configuration.style {
            path = draw(samples: samples, on: context, path: path, with: configuration, lastOffset: lastOffset, sides: .both, dur: duration)
        } else {
            path = draw(samples: samples, on: context, path: path, with: configuration, lastOffset: lastOffset, sides: .up, dur: duration)
            path = draw(samples: samples.reversed(), on: context, path: path, with: configuration, lastOffset: lastOffset, sides: .down, dur: duration)
        }
        path.closeSubpath()
    ...
    private func draw(samples: [Float], on context: CGContext, path: CGMutablePath, with configuration: Waveform.Configuration, lastOffset: Int, sides: Sides, dur: Float) -> CGMutablePath {
        let graphRect = CGRect(origin: CGPoint.zero, size: configuration.size)
        let positionAdjustedGraphCenter = 0.5 * graphRect.size.height
        ...
        for (index, sample) in samples.enumerated() {
            let adjustedIndex: Int
                if index % Int(offset) == 0 {
                    s_Samples.append(Double(sample))
                    count += 1
                }
