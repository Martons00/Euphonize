...
extension WaveformImageDrawer {
    ...
    func draw(on context: CGContext, from samples: [Float], with configuration: Waveform.Configuration, renderer: WaveformRenderer) {
        ...
        var dur : Float = 0.0
        do{
            dur = try Float(AVAudioPlayer(contentsOf: audioUrl!).duration)
        } catch {
            print("Mannag")
        }
        renderer.render(samples: samples, on: context, with: configuration, lastOffset: lastOffset, duration: dur)
    }
